...
<PredictBeforeReveal
  ...
  question="정확도 99%인 출력과 80%인 출력이 있다. 어느 쪽을 사람이 일일이 확인해야 할까?"
  options={[
    { key: 'a', label: '99% — 나머지 1%가 숨어 있어 더 위험하다' },
    { key: 'b', label: '80% — 틀릴 확률이 높으니 당연히' },
    { key: 'c', label: '정확도만으로는 정할 수 없다' },
    { key: 'd', label: '둘 다 사람이 봐야 한다' },
  ]}
  answer="c"
  revealTitle="정확도는 세 변수 중 하나일 뿐이다"
  revealBody="99%라도 나머지 1%가 되돌릴 수 없는 사고라면 전수로 확인해야 하고, 80%여도 틀린 걸 즉시 알아채 쉽게 고칠 수 있으면 그냥 쓴다. 확인할지 말지는 정확도가 아니라 회복 비용과 검증 비용의 관계에서 나온다 — 아래에서 그 셋을 같이 놓는다."
/>
...
<section class="log" aria-label="변경 로그">
  ...
  <ul>
    ...
    <li><time>2026-07-26</time><span>Decision 타입 정리 — 본문을 읽기 전에 예측하는 지점 추가(정확도만으로 판단할 수 없다는 지점). 나머지 요건(완료 조건·출처)은 다음 갱신에</span></li>
