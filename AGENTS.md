...
| `knowledge` | ⑤ 지식·업무 시스템 | 노트가 실행 시스템이 되는가 — Obsidian 4편, 아직 **운영 전**이라 근거는 `researched` |
...
독자 우선순위: ① 미래의 나 ② 판단력을 보는 사람(채용·협업 신호) ③ 같은 경로를 걸으려는 실무자. **①②를 잃으면서 ③을 늘리는 결정은 하지 않는다** — 판단 기준이 빠진 채 절차만 남은 콘텐츠는 이 제품이 아니다.

다만 **콘텐츠 깊이와 입구 난이도는 별개다.** 진입장벽을 낮추는 것(실제 주제 병기·"처음이라면" 큐레이션·읽는 순서·메타데이터 접기)은 ①②를 잃는 일이 아니라 ③을 위해 입구만 넓히는 일이다.
...
작업 중 받은 정정을 규칙으로 남긴다(최신순). **손으로 고치지 말고 `/learn` 으로 append 한다** — 날짜와 중복 처리를 헬퍼가 맡는다.

<!-- LEARN:ANCHOR -->
- (2026-07-26) 공개 화면을 만들 때는 내부 분류어만 노출하지 말고, 실제 주제·도구명과 시작 지점을 함께 보여준다.
- (2026-07-26) 원칙을 스키마·검증으로 강제하기 전에 그 규칙이 막아버릴 정당한 작업이 있는지 확인하고, 있으면 막지 말고 표시로 푼다.
<!-- CC-RULES:END -->
